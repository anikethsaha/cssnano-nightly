...
describe("E2E cssnano ", () => {
  ...
  it("should install the latest nightly version" + version, async done => {
    expect(
      shell.exec(
        "npm install cssnano-nightly@" + version + " --registry " + registryUrl
      ).code
    ).toBe(0);
    done();
  });
  ...
  it(
    "should minify the input using postcss cli and cssnano" + version,
    async done => {
      ...
      expect(outputCode.split("\n").length).toBe(1);
       expect(outputCode.split("").length > 1).toBe(true);
      done();
    }
  );
});
